...
menu model =
    Navbar.config NavMsg
        ...
        |> Navbar.items
            [ Navbar.itemLink [ href "#tasks" ] [ text "Tasks" ]
            , Navbar.itemLink [ href "#show" ] [ text "Item 2" ]
            ]
        |> Navbar.view (Debug.log "Navbar Item Tasks" model.navState)
...
update msg model =
    case msg of
        ...
        NavMsg state ->
            case state of
                _ ->
                    ( { model | navState = Debug.log "State -------> " state }
                    , Cmd.none
                    )
